...
//pesquisados
/*
const maxDigits = document.getElementById('altura').getAttribute('max').length;

document.getElementById('altura').addEventListener('input', (event) => {
  const input = event.target;
  const value = input.value.replace(/\D/g, ''); // Remove tudo que não é um dígito
  if (value.length > maxDigits) {
    input.value = value.slice(0, maxDigits); // Corta a entrada se ela exceder o limite
  }
});
*/
